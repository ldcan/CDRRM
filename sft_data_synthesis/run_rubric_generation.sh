...
#       <sampled_jsonl>      \   # raw pairwise data (e.g. rubric_synthesis/data/sampled.jsonl)
#       <rubric_pair_jsonl>  \   # rubric_synthesis output: rubric_pair.jsonl
...
SAMPLED_JSONL="${1:-../rubric_synthesis/data/rubric_generation_sampled_data.jsonl}"
RUBRIC_PAIR_JSONL="${2:-../rubric_synthesis/output/rubric_pair.jsonl}"
...
if [[ -z "$SAMPLED_JSONL" ]] || [[ -z "$RUBRIC_PAIR_JSONL" ]]; then
    ...
    echo "  sampled_jsonl:     raw pairwise data (e.g. rubric_synthesis/data/sampled.jsonl)"
    echo "  rubric_pair_jsonl: rubric_synthesis output (rubric_pair.jsonl)"
    exit 1
fi
